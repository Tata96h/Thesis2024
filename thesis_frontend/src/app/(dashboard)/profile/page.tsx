...
const DropdownUser = () => {
  ...
  useEffect(() => {
    ...
    if (storedUserInfo) {
      ...
      if (parsedUserInfo.utilisateur_id && parsedUserInfo.role === 2) {
        const fetchUserInfo = async () => {
          setLoading(true);
          const url = "http://127.0.0.1:8000/enseignants/{utlisateur_id}?utilisateur_id=${parsedUserInfo.utilisateur_id}";
          ...
          try {
            ...
            console.log('Données récupérées pour utilisateur', parsedUserInfo.utilisateur_id, data);
            // Ajouter les données récupérées à localStorage
            localStorage.setItem(
              "enseignantInfo",
              JSON.stringify(data)
            );
              
          // Vérifier que les données ont été ajoutées à localStorage
          console.log(localStorage);
          
            
            // setUserInfo({ ...parsedUserInfo, additionalInfo: data });
          } catch (error) {
